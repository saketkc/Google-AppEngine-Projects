...
all_list= {"acads":acad_id,"cult":cult_id,"sports":sports_id,"hostel":hostel_id,"tech":tech_id}
	...
	for key in all_list.keys():
        ...
        for i, an_event in zip(xrange(len(feed.entry)), feed.entry):
            ...
            for a_when in an_event.when:
                #all_events += " : " + str(a_when.start_time) +" - " + str(a_when.end_time) #+ " @ " + str(an_event.where.text)
...
def InsertEvent(title='Tennis with Beth', content='Meet for a quick lesson', where='On the courts', start_time=None, end_time=None,category=None, recurrence_data=None):
    ...
    event.timezone = gdata.calendar.Timezone(value="Asia/Calcutta")
...
class MainPage(webapp.RequestHandler):
    def get(self):
        ...
        event_start_time = time.strftime('%Y-%m-%dT%H:%M:%S.000+05:30Z', datetime.datetime(start_year,start_month,start_date_date,start_hour,start_minute,start_second).timetuple())
        event_end_time = time.strftime('%Y-%m-%dT%H:%M:%S.000+05:30Z', datetime.datetime(end_year,end_month,end_date_date,end_hour,end_minute,end_second).timetuple())
        ...
        self.response.out.write('Succesfully added Event ')
